        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 1rem" }}>
          ...
          <FormField label="Problem Category">
            <select className="input-field" value={f.category} onChange={e => up("category", e.target.value)} disabled={isSubmitting}>
              ...
              {["AI / ML", "Web3 / Blockchain", "IoT / Hardware", "Coding competition" , "HealthTech", "FinTech", "EdTech", "Climate", "Open"].map(c => <option key={c}>{c}</option>)}
            </select>
          </FormField>
        </div>
...
function ProjectForm({ onSubmit, onBack, isSubmitting }) {
  // Added projectDate and lastDateOfApply to state
  const [f, setF] = useState({ title: "", desc: "", usestack: [], needstack: [], type: "", status: "", githubRepo: "", projectDate: "", lastDateOfApply: "" });
  const [error, setError] = useState("");
  ...
  const handleSubmit = () => {
    ...
    if (!f.projectDate) return setError("Project Date is required.");
    if (!f.lastDateOfApply) return setError("Last Date of Apply is required.");
    if (f.usestack.length === 0) return setError("Please add at least one Tech Stack.");
    if (f.needstack.length === 0) return setError("Please add at least one Required Skill.");
      ...
      <div className="glass" style={{ padding: "2rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 1rem" }}>
          <FormField label="Project Type">
            <select className="input-field" value={f.type} onChange={e => up("type", e.target.value)} disabled={isSubmitting}>
              ...
              {["Web App", "Mobile App", "AI / ML", "Blockchain", "Coding competition" ,"Desktop App", "API / Backend", "Open Source", "Research"].map(x => <option key={x}>{x}</option>)}
            </select>
          </FormField>
          <FormField label="Project Status">
            <select className="input-field" value={f.status} onChange={e => up("status", e.target.value)} disabled={isSubmitting}>
              <option value="">Select status</option>
              {["Idea Phase", "In Progress", "Beta", "Completed", "Application open" ,"Looking for Contributors"].map(x => <option key={x}>{x}</option>)}
            </select>
          </FormField>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 1rem" }}>
          <FormField label="Project Date">
            <input type="date" className="input-field" value={f.projectDate} onChange={e => up("projectDate", e.target.value)} disabled={isSubmitting} />
          </FormField>
          <FormField label="Last Date to Apply">
            <input type="date" className="input-field" value={f.lastDateOfApply} onChange={e => up("lastDateOfApply", e.target.value)} disabled={isSubmitting} />
          </FormField>
        </div>
      ...
      if (localtoken) {
        ...
        if (type.toLocaleLowerCase() === 'project') {
          ...
          const responce = await fetch(url, {
            ...
            // Added projectDate and lastDateOfApply to the body payload
            body: JSON.stringify({ 
              ProjectTitle: formData.title, 
              ProjectDescription: formData.desc, 
              ProjectType: formData.type, 
              ProjectStatus: formData.status, 
              AllTechStack: formData.usestack, 
              RequiredSkills: formData.needstack, 
              ProjectRepoLink: formData.githubRepo,
              projectDate: formData.projectDate,
              lastDateOfApply: formData.lastDateOfApply
            })
          });
          const data = await responce.json()
          if (data.status) {
